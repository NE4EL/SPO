"""
End-to-end тест полного цикла бизнес-логики.

Сценарий:
  1. Администратор входит в систему
  2. Администратор регистрирует менеджера и механика
  3. Менеджер входит и создаёт клиента, автомобиль, запчасти
  4. Менеджер создаёт заказ-наряд и назначает механика
  5. Механик видит только свой заказ
  6. Менеджер добавляет запчасть в заказ и завершает его
  7. Проверяем все эффекты бизнес-логики:
     - остаток на складе уменьшился
     - создалась складская операция (OUT)
     - создался алерт о низком остатке
     - заказ получил статус completed и стоимость
"""
import pytest
